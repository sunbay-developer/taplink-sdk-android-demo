...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            Toast.makeText(this, "Application startup failed: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun initPaymentService() {
        try {
            ...
            Toast.makeText(this, "Payment service initialization failed: ${e.message}", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun updateTransactionButtonsState() {
        try {
            ...
            Log.d(TAG, "Update transaction buttons state - Connected: $connected, HasAmount: $hasAmount")
        } catch (e: Exception) {
    ...
    private fun startSalePayment(
        ...
        Log.d(TAG, "Start SALE payment - Amount: $selectedAmount, SurchargeAmount: $surchargeAmount, TipAmount: $tipAmount, TaxAmount: $taxAmount, CashbackAmount: $cashbackAmount")
        ...
        Log.d(TAG, "Creating SALE transaction - RequestId: $transactionRequestId, OrderId: $referenceOrderId")
...
//        }
        ...
        Log.d(TAG, "Creating transaction - RequestId: $transactionRequestId, OrderId: $referenceOrderId")
    ...
    private fun handlePaymentSuccess(result: PaymentResult) {
        ...
        if (requestId != null && requestId.isNotEmpty()) {
            ...
            Log.w(TAG, "transactionRequestId is null or empty, trying to find by latest PROCESSING status")
    ...
    private fun handlePaymentFailure(transactionRequestId: String, code: String, message: String) {
        ...
        if (!updated) {
            Log.e(TAG, "Failed to update transaction status - transactionRequestId not found: $transactionRequestId")
            // If the transactionRequestId returned by the SDK does not exist, try to find all transactions in PROCESSING status and update the latest one
            val processingTransactions = TransactionRepository.getTransactionsByStatus(TransactionStatus.PROCESSING)
            if (processingTransactions.isNotEmpty()) {
                val latestProcessing = processingTransactions.first()
    ...
    private fun retryPaymentWithSameId() {
        lastTransactionType?.let { transactionType ->
            ...
            val failedTransactions = TransactionRepository.getTransactionsByStatus(TransactionStatus.FAILED)
    ...
    private fun handleQueryResult(originalTransaction: Transaction, result: PaymentResult) {
        Log.d(TAG, "Query result - Status: ${result.transactionStatus}, ID: ${result.transactionId}")
        ...
        when (result.transactionStatus) {
            ...
            "FAILED" -> {
                ...
                AlertDialog.Builder(this)
                    ...
                    .setMessage("Transaction was confirmed as failed:\n" +
                            "Amount: ${amountFormatter.format(originalTransaction.amount)}\n" +
            ...
            "PROCESSING" -> {
                ...
                AlertDialog.Builder(this)
                    ...
                    .setMessage("Transaction is still being processed:\n" +
                            "Amount: ${amountFormatter.format(originalTransaction.amount)}\n" +
                            "Transaction ID: ${result.transactionId ?: "N/A"}\n" +
    ...
    private fun executePaymentWithId(
        ...
        Log.d(TAG, "Executing payment with specific ID - Type: $transactionType, RequestId: $transactionRequestId")
        ...
        when (transactionType) {
            TransactionType.SALE -> {
                paymentService.executeSale(
                    ...
                    amount = selectedAmount,
                    currency = "USD",
                    description = "Demo SALE Payment (Retry) - ${amountFormatter.format(selectedAmount)}",
                    surchargeAmount = null,
                    tipAmount = null,
                    taxAmount = null,
                    cashbackAmount = null,
            ...
            TransactionType.AUTH -> {
                paymentService.executeAuth(
                    ...
                    amount = selectedAmount,
                    currency = "USD",
                    description = "Demo AUTH Payment (Retry) - ${amountFormatter.format(selectedAmount)}",
                    callback = callback
                )
            }
            TransactionType.FORCED_AUTH -> {
        ...
        try {
            if (::paymentService.isInitialized) {
                ...
                if (isConnected) {
                    ...
                    // If not connected, update status and try to reconnect
                    updateConnectionStatus("Not Connected", false)
                    
                    // Attempt automatic reconnection
                    Log.d(TAG, "Connection lost, attempting automatic reconnection...")
                    connectToPaymentService()
                }
            }
        } catch (e: Exception) {
        ...
        when (requestCode) {
            REQUEST_CODE_CONNECTION -> {
                if (resultCode == ConnectionActivity.RESULT_CONNECTION_CHANGED) {
                    ...
                    Log.d(TAG, "Connection configuration changed - Mode: $connectionMode, Message: $connectionMessage")
                    ...
                    // Update connection status display
                    updateConnectionStatus(connectionMessage ?: "Connection configuration updated", true)
                }
            }
