...
fun AuthTransactionPage(
    ...
    var description by remember { mutableStateOf("预授权交易") }
            ...
            Button(
                onClick = {
                    ...
                    val amountValue = try {
                        ...
                        viewModel.addLog("错误: 金额格式不正确，请输入有效的数字")
                        return@Button
                    }
                    
                    if (amountValue <= BigDecimal.ZERO) {
                        viewModel.addLog("错误: 金额必须大于0")
                        return@Button
                    }
                ...
                Text(if (isProcessing) "处理中..." else "发起预授权")
            }
            
            // 说明文字
            Text(
                text = "预授权用于冻结一定金额，后续可通过预授权完成或增量授权进行操作",
...
private fun performAuth(
    ...
    viewModel.addLog("发起预授权，金额: $$amount")
    
    val merchantOrderNo = "AUTH_${System.currentTimeMillis()}"
    ...
    TaplinkSDK.execute(request, object : PaymentCallback {
        override fun onProgress(event: PaymentEvent) {
            viewModel.addLog("预授权进度: ${event.eventMsg}")
        }
        
        override fun onSuccess(result: PaymentResult) {
            viewModel.addLog("预授权成功!")
            viewModel.addLog("  - 交易ID: ${result.transactionId}")
            viewModel.addLog("  - 商户订单号: ${result.merchantOrderNo}")
            viewModel.addLog("  - 授权码: ${result.authCode}")
        ...
        override fun onFailure(error: PaymentError) {
            viewModel.addLog("预授权失败: ${error.message}")
